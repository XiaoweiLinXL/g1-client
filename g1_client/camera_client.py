...
Images are sent at native camera resolution (480x640) with no intermediate
resize, so the server's resize_with_pad matches the training path exactly.
...
class CameraClient:
    ...
    def _resize_jpeg(bgr: np.ndarray) -> bytes:
        """JPEG-encode in cv2's native BGR order at native camera resolution.
